...
public final class MemorySearchExample {
    ...
    private void run() throws Exception {
        ...
        // Create searcher
        final MolSearch searcher = new StandardizedMolSearch();
        searcher.setQuery(query);
        searcher.setTarget(target);

        // Create and set search options
        final MolSearchOptions options = new MolSearchOptions(SearchConstants.SUBSTRUCTURE);
     ...
     * @throws Exception
     */
    private void printHitsWithFindNext(final MolSearch molSearch) throws Exception {
     ...
     * @throws Exception
     */
    private void printHitsWithFindAll(final MolSearch molSearch) throws Exception {
        final int[][] allHits = molSearch.findAll();
        if (allHits == null) {
            out.println("No hits has been found.");
            return;
        }
        for (final int[] currHit : allHits) {
